...
from peft import (
    PromptTuningConfig, # imports the configuration for Prompt Tuning
    PromptTuningInit, # imports prompt init types
...
peft_config = PromptTuningConfig(
    ...
    prompt_tuning_init_text="Let's think step by step. At the end, you MUST write the answer as an integer after '####'.",
    tokenizer_name_or_path=sft_model_name,
)
...
for qna in gsm8k_train: # Iterates over the GSM8K training data
    ...
    eot_token = "<|eot_id|>"
    if eot_token in train_sample:
        train_sample = train_sample[train_sample.index(eot_token) + len(eot_token):]
...
def format_prompt(chats: list) -> str:
    try:
        return sft_tokenizer.apply_chat_template(chats, tokenize=False)
    except Exception:
